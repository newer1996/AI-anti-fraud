...
class LLM:
    ...
    def generate(self, question: str, context: List):
        if context:
            prompt = f'''假如你的角色是反诈骗助手，你会按照要求做出回复。
            你会对比提供的'参考诈骗案例'和'问题'，从逻辑的角度分析两者的相似性。
            注意:'参考诈骗案例'提供的是诈骗案例或者诈骗话术。
            注意:如果'参考诈骗案例'与'问题'在逻辑上存在相似性，则'结论'是：'问题''是诈骗信息'。
            注意：请你先给出'结论'(结论为'是'或者'不是'诈骗信息)，然后给出你的'分析过程'，最后列出'参考诈骗案例'。
            现在请你根据以上要求，阅读下面的'参考诈骗案例'，然后对'问题：{question}'作出回答。如果'参考诈骗案例'为空，那么直接回答'问题：{question}'。
            '参考诈骗案例：{context}'
            '''
        else:
            prompt = question
